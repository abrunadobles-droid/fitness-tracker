...
**Update 2026-09-09 — causa CONFIRMADA:** el cron del 09-sep (17:01 UTC) ya muestra el cuerpo de WHOOP: `{"error":"invalid_client","error_description":"Client authentication failed (e.g., unknown client, no client authentication included, or unsupported authentication method)"}`. No es el refresh token (eso sería `invalid_grant`): **los secrets `WHOOP_CLIENT_ID` / `WHOOP_CLIENT_SECRET` de GitHub no coinciden con la app** (valor mal pegado, con comillas/espacios, o de otra app). El `--auth` local con `secrets.toml` sí funciona (mismo método, secret en el body), así que las credenciales correctas son las del TOML. Fix: subir ambos secrets desde `config` por stdin (pasos abajo).
