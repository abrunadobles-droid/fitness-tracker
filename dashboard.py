...
def get_monthly_data(year, month):
    ...
    try:
        ...
        # --- HR Zones + Sleep: WHOOP cache (primary) with Garmin fallback ---
        whoop_ok = False
        try:
            ...
            if cache_key in whoop_cache:
                ...
                if wc.get('sleep_hours_avg', 0) > 0:
                    data['sleep_hours_avg'] = round(wc['sleep_hours_avg'], 1)
                    data['days_before_930'] = wc.get('days_before_930', 0)

                whoop_ok = True
                synced = wc.get('synced_at', '?')
                print(f"   [WHOOP CACHE] {cache_key} — zones 1-3: {data['hr_zones_1_3']}h, 4-5: {data['hr_zones_4_5']}h (sync: {synced})")
            else:
                print(f"   [WHOOP CACHE] No hay datos para {cache_key} — usando Garmin como fallback")
        except Exception as e:
            print(f"   [WHOOP CACHE] Error: {e} — usando Garmin como fallback")
